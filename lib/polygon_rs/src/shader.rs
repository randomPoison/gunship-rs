use gl::{Program, ProgramError, Shader, ShaderError, ShaderType};
use std::fs::File;
...
#[derive(Debug)]
pub struct ShaderProgram(Program);

impl ShaderProgram {
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<ShaderProgram, Error> {
        let mut file = File::open(&path)?;
        let mut contents = String::new();
        file.read_to_string(&mut contents)?;

        let programs = ShaderParser::parse(&*contents)?;
        let vert_src =
            programs
            .iter()
...
impl ShaderParser {
    pub fn parse(shader_src: &str) -> Result<Vec<ShaderProgramSrc>, Error> {
